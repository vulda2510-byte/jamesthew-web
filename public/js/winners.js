document.addEventListener("DOMContentLoaded", () => {
    ...
    async function loadWinners() {
        ...
        try {
            ...
            const res = await fetch(`/api/v1/contests/winners?${params}`);
            ...
            if (result.success && result.data && result.data.length > 0) {
                tbody.innerHTML = result.data.map(w => {
                    ...
                    // Sửa w.winner?.username -> w.user?.username
                    const username = w.user?.username || w.winner?.username || 'Thành viên';

                    return `
                        <tr>
                            <td class="py-3">${rankBadge}</td>
                            <td class="text-start fw-bold py-3">@${username}</td>
